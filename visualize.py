...
class SliceScroller(object):
    ...
    def update(self):
        ...
        if not isinstance(self.ddts[self.ind], np.ndarray):
            ...
            self.ddts[self.ind] = np.sqrt(
                self.cddt.slices[self.ind].make_ddt(True)
            ).transpose()
        ...
        self.ax1.set_title(
            "DDT - Reconstructed from a slice of the PCDDT, compression factor: "
            + str(compression_factor)
        )
        self.ax1.set_ylabel("Theta = %s" % self.cddt.slices[self.ind].theta)
        self.ax1.imshow(
            self.ddts[self.ind], cmap="gray", interpolation="nearest", aspect="auto"
        )
...
if __name__ == "__main__":
    ...
    w = 1350
    img = np.power(
        cddt.slices[3].make_ddt(reversed_dir=True).transpose()[120 : 120 + w, :w], 0.7
    )
